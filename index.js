    ...
    */
    ...
    db.all(query, (err, rows) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Erro ao buscar cadastros." });
        }
    ...
    if (tu_nome) {
        ...
        db.all(query, [`%${tu_nome}%`], (err, rows) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: "Erro ao buscar turmas." });
            }
...
app.post("/login/aluno", (req, res) => {
    ...
    const sql = `SELECT * FROM Aluno WHERE al_email = ? AND al_senha = ?`;
...
app.post("/login/prof", (req, res) => {
    ...
    const sql = `SELECT * FROM Prof WHERE pr_email = ? AND pr_senha = ?`;
